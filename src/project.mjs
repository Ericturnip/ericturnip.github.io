...
const stories = {
  ...
  heliophysics: {
    ...
    resources: material('/downloads/cospar-2026-kp-forecast.pdf', 'Kp forecast abstract · PDF') + material('/downloads/cospar-2026-solar-jets.pdf', 'Solar jets abstract · PDF') + material('https://github.com/Ericturnip/SolarResearch', 'PUNCH processing code · GitHub'),
  ...
  roman: {
    ...
    body: section('Connecting structures and optical performance', `<p>Telescope performance depends on more than the optics alone. Structural motion, thermal effects, and vibration can affect the measurements an observatory makes.</p><p>During my internship with the Roman Space Telescope team at NASA Goddard, I worked on integrated modeling workflows involving structural dynamics and jitter analysis.</p>`)
      + section('Making simulations easier to follow', `<p>I implemented real-time log streaming for the Structural, Thermal, and Optical Performance (STOP) pipeline, enabling continuous remote monitoring of simulation progress.</p><p>This work made long-running simulations easier to follow as they ran, bringing their progress into view without waiting for the complete run to finish.</p>`)
      + section('Improving the analysis workflow', `<p>I optimized MATLAB-based JittTool workflows for structural dynamics and jitter analysis. The work reduced analysis runtime by 50%, as documented in my résumé.</p>`)
      + section('Presentation', `<div class="coming-soon"><span class="eyebrow">ROMAN SPACE TELESCOPE</span><h3>Summer internship presentation</h3><p>Slides coming soon.</p></div>`),
    resources: material('/downloads/eric-chen-resume.pdf', 'Experience summary · Résumé PDF'),
    note: 'Presentation slides forthcoming',
  },
  foxit: {
    ...
    body: section('From a prompt to a useful workflow', `<p>Language-model applications become more useful when they can act through existing software APIs. They also become harder to evaluate: choosing the right tool is only one part of completing a multistep task.</p><p>At Foxit, I worked on connecting Qwen to the Foxit API suite through the Model Context Protocol (MCP).</p>`)
      + section('What I built', `<p>I developed an MCP bridge server that enabled the model to invoke tools across Foxit APIs and carry out multistep workflows. This connected the model’s tool-use decisions to the software operations needed to execute them.</p>`)
      + section('Evaluating tool use', `<p>I built a local evaluation framework to test tool selection and multistep orchestration. The focus was on the complete workflow: how the model chooses tools and coordinates their execution across a task.</p><p>The project brought together API integration, model tooling, and evaluation, with an emphasis on making behavior observable and testable.</p>`),
    resources: material('/downloads/eric-chen-resume.pdf', 'Experience summary · Résumé PDF'),
